...
const patrimoineProvider: DataProvider = {
    getList: async (resource: string, params: any) => {
        ...
        if (resource === 'patrimoines') {
            ...
            queryParams += `/patrimoines/?nom_patrimoine=${id}/${resource}`
        }
    ...
    getOne: async (resource: string, params: CustomGetOneParams) => {
        ...
        if (resource === 'patrimoines') {
            queryParams = `${resource}/?nom=${idPatrimoine}`;
        } else if (resource === 'possessions') {
            queryParams = `patrimoines/?nom_patrimoine=${idPatrimoine}/${resource}/?nom_possession=${idPossession}`;
        }
        const response = await conf.get(`${apiUrl}/${queryParams}`);
        const dataWithId = {
            ...response.data,
    ...
    update: async (resource: string, params: { data: Patrimoine ,id:number}) => {
        ...
        const response = await conf.put(`${resource}${queryParams}`, data);
        return {
    ...
    create: async (resource: string, params: { data: Patrimoine }) => {
        ...
        if (resource === 'patrimoines') {
            ...
            queryParams += `/patrimoines/?nom_patrimoine=${data.nom}/${resource}`
        }
